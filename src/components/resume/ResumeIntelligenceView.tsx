...
export default function ResumeIntelligenceView({
  ...
  }, [report.recommendations, selectedPriority, selectedCategory]);
  ...
  const handleDismiss = (recId: string) => {
    setDismissedIds(prev => [...prev, recId]);
    setActionNotice({
      type: "info",
  ...
  return (
    <div className="w-full space-y-6 animate-fadeIn">
      ...
      {/* Section 22: Resume Sections Analyzed Checklist */}
      <div className="bg-white/80 backdrop-blur-md border border-slate-200 p-5 rounded-3xl shadow-sm space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2.5">
          {report.sections.map((sec, idx) => (
            <div key={idx} className="p-2.5 rounded-xl border border-slate-100 bg-slate-50/60 text-xs space-y-1">
      ...
      {/* Recommendations & Actionable Findings Filter Bar */}
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div>
            <h3 className="text-slate-800 font-display font-bold text-lg">
              Actionable Recommendations
            </h3>
            <p className="text-slate-500 text-xs">
              Prioritized improvements based on verified resume evidence and ATS scanning rules.
            </p>
          </div>
        ...
        {filteredRecommendations.length === 0 ? (
          <div className="p-10 text-center bg-white/70 backdrop-blur-md rounded-3xl border border-slate-200 space-y-2">
            ...
            <h4 className="font-display font-bold text-sm text-slate-800">
              No recommendations found in this filter.
            </h4>
            <p className="text-slate-500 text-xs">
              All detected issues in this priority tier have been addressed or dismissed.
            </p>
          </div>
        ) : (
          <div className="space-y-3">
                    ...
                    <div className="p-4 bg-cyan-50/70 border border-cyan-200 rounded-2xl space-y-3 animate-fadeIn">
                      ...
                      <div className="flex justify-end gap-2">
                        <button
                          ...
                          className="px-3 py-1.5 text-slate-600 text-xs font-semibold hover:bg-white rounded-lg"
                        >
                          Cancel
                        </button>
                        <button
                          ...
                          className="px-4 py-1.5 bg-cyan-600 hover:bg-cyan-700 text-white text-xs font-bold rounded-xl shadow-sm"
                        >
                          Save Changes
                        </button>
                      </div>
                      ...
                      <div className="pt-2 text-[11px] text-slate-400 flex items-center justify-between">
                        ...
                        {rec.status === "ACTIVE" && (
                          <button
                            type="button"
                            onClick={() => handleApply(rec)}
                            className="px-3 py-1 bg-cyan-500 hover:bg-cyan-600 text-white font-bold rounded-lg text-xs"
                          >
